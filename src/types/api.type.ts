...
export interface EventType {
  ...
  locationType: VideoConferencingPlatform;
    // ✅ NUEVOS CAMPOS - Calendarios específicos
  /** ID del calendario donde se crearán las reuniones (ej: "primary", "work@company.com") */
  calendar_id?: string;
  
  /** Nombre descriptivo del calendario asignado */
  calendar_name?: string;
  createdAt: string;
  updatedAt: string;
  user: UserType;
...
export interface UserAvailabilityResponseType {
  availability: {            // ← Cambiar a "availability"
    days: DayAvailabilityType[];
    timeGap: number;
  };
  message: string;
...
export interface MeetingType {
  ...
  status: MeetingStatus;
    // ✅ NUEVO CAMPO - Referencia al calendario usado
  /** ID del calendario donde se creó la reunión */
  calendar_id?: string;
  createdAt: string;
  updatedAt: string;
  event: EventType;
